    ...
    */
    ...
    instance.events.MarketTransaction()
      .on('data', (event) => {
        ...
        if (eventType == "Buy") {
          alert_msg('Succesfully Fish purchase! Now you own this Fish with TokenId: ' + tokenId, 'success')
        }
        if (eventType == "Create offer") {
          alert_msg('Successfully Offer set for Fish id: ' + tokenId, 'success')
        ...
        if (eventType == "Remove offer") {
          alert_msg('Successfully Offer remove for Fish id: ' + tokenId, 'success')
...
function createKitty() {
  ...
  let res;
  try {
    res = instance.methods.createFishGen0(dnaStr).send();
  } catch (err) {
    console.log(err);
  }
}
